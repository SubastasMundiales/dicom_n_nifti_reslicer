...
Read the coronal dicom scan and create the sagital and axial slices
11/2022
by: Aranca2504
"""
...
fpath = '/Users/macbook/Desktop/dcm_scan_loader/coronal'
# load the DICOM files
files_id = next(os.walk(fpath))[2]
print(files_id)
files = []
path_file = 'coronal/' + files_id[0]
for fname in files_id:
    r,extension = os.path.splitext(fname)
    if extension == '.dcm':
        print("loading: {}".format(fname))
        files.append(pydicom.dcmread('coronal/' + fname))
print("file count: {}".format(len(files)))

#SKIP FILES WIRH NO SLICELOCATION
slices = []
skipcount = 0
for f in files:
...
# pixel aspect (y/x), assuming all slices are the same
ps = slices[0].PixelSpacing
ss = slices[0].SliceThickness
ax_aspect = ss/ps[0]
sag_aspect = ps[1]/ss
cor_aspect = ps[1]/ps[0]
